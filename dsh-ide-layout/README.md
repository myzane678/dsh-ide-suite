...
### 会话内容宽度（白色会话区横向可拉伸）
- 插件自建双手柄：**常驻灰竖线贴在白色会话区左右边框上**（3px 灰线压在白缘内侧，8px 命中带跨坐白缘，`cursor:col-resize`，hover 加深）
- **竖线按可见白卡分段**：只在输入框以上、视口内白卡的两侧画线，**白卡之间的 gap 不画**（滚动时实时跟随）
- **对称缩放**：拖任一缘、两缘等量外扩/内收（外拖一格宽涨两格）；内容列 `max-width + margin:0 auto` 恒居中，**竖向中心线始终落在 agent 区中间，不脱开**
- 手柄每帧按**实测白缘**归位（宿主重建 DOM / 拉宽拉窄 / 侧栏拖动都不脱位）；欢迎页与轨迹页没有内容列，手柄自动隐藏
- 宽度写入宿主 ConversationRoot 根元素的 `--dsh-chat-user-width` + localStorage（`dsh.conversation.contentWidth`）跨会话记忆；下限 640px、硬上限 1200px，动态上限 = 根宽 − 176px（每侧 88px 安全区，防手柄被挤出列外拖不回来）
- 宿主持有者自带的宽度手柄继续隐藏：其定位公式以卡片中心为基准、与白缘有偏差，且在本环境下不产出可见输出，故由本插件接管
- ⚠️ 已知问题（下版本修复）：非 hover 状态下仅最底部白卡的竖线清晰（常驻色对比度过低 + 颜色事件式粘滞），不影响拖拽
